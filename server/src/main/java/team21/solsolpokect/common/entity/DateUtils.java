...
public class DateUtils {
    ...
    public static LocalDate getStartOfMonthDate(String year, String month) {
        int intYear = Integer.parseInt(year);
        int intMonth = Integer.parseInt(month);

        return LocalDate.of(intYear, intMonth, 1); // 해당 년도와 월의 1일을 반환
    }

    public static LocalDate getEndOfMonthDate(String year, String month) {
        int intYear = Integer.parseInt(year);
        int intMonth = Integer.parseInt(month);

        YearMonth yearMonth = YearMonth.of(intYear, intMonth);
        return LocalDate.of(intYear, intMonth, yearMonth.atEndOfMonth().getDayOfMonth()); // 해당 년도와 월의 마지막 날짜를 반환
    }
}
